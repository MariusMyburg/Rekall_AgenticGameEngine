...
- AI-authored gameplay completion must be proven with deterministic `rekall.runtime.inspect_scene` input frames and generic entity/component/transform assertions after the latest scene or module mutation. A clean build, source inspection, soak, capture, playable adapter, package, or package audit does not prove that world gameplay is attached or effective.
