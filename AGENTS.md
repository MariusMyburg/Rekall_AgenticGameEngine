...
- AI-authored modules should use generic mutation helpers such as `ReplaceEntity`, `RemoveEntity`, `UpdateEntity`, `UpdateEntitiesWithTag`, `UpdateEntitiesWithComponent`, `UpdateEntitiesWithTagAndComponent`, `WithTag`, `WithoutTag`, `WithVisible`, `WithComponentNumber`, `WithComponentBoolean`, and `WithComponentString` instead of manually rebuilding `world.Entities` or mutable JSON when ordinary entity updates are enough. Read agent-owned component state with `ComponentNumber`, `ComponentBoolean`, and `ComponentString`.
...
- AI-authored gameplay completion must be proven with deterministic `rekall.runtime.inspect_scene` input frames and generic entity/component/transform assertions after the latest scene or module mutation. Require representative input, an attached agent-owned `Game.*` component, and a strict executable assertion showing either a nonzero transform delta or a changed agent-owned component property. When an assertion fails, repair the authored behavior and rerun the intended assertion; do not weaken it merely to obtain a pass. A clean build, source inspection, soak, capture, playable adapter, package, package audit, or existence-only assertion does not prove that world gameplay is attached or effective.
- Gameplay checkpoint enforcement must keep the bounded scene/entity/component and module-repair tools available that an agent needs to construct the checkpoint prerequisites. Defer validation, polish, capture, packaging, audit, and unrelated delivery work; do not deadlock a valid module-first authoring order by demanding scene evidence while blocking scene authoring.
