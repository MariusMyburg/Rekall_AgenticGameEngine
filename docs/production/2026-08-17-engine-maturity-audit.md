...
| Animation | Implemented | versioned inline/catalog clips, scalar/vector/color/string tracks, interpolation, loop/clamp/ping-pong, bounded asset/track/key/marker work with structured diagnostics, deterministic malformed corpus, 7,200-frame resume and per-frame-observation proofs, events, generic property mutation, runtime-state proof | cross-fade/blend layers, skeletal GLB tracks/skinning, state graphs |
