...
Codex project runs now bind the external AGE MCP host to the selected project through the structured process arguments `mcp`, `stdio`, `--project-root`, and the normalized absolute root. The scoped host uses the same extracted project-scope contract as embedded Ollama/OpenAI agents: omitted direct or gateway-target roots are defaulted, while a different absolute root returns `REKALL_AGENT_PROJECT_SCOPE_VIOLATION` before the command registry or mutation handler runs. Explicit plain `mcp stdio` remains unscoped.

Behavioral coverage preserved the ordinary Prism Relay-style MCP path by executing the real `rekall.project.create` command through an unscoped server. No real authenticated Codex rerun was necessary because the retained Prism Relay artifacts and gameplay proof are unchanged; this fix narrows command authority without changing authored content. Focused MCP/project-session/Codex-runner/CLI verification passed 51/51, the one broader engine suite passed 2,075/2,075, and the solution build completed with zero warnings and errors.
