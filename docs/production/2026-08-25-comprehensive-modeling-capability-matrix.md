...
| Bevel/chamfer | Partial | `bevel_edges`, graph and functional modifier-stack nodes; arbitrary selected two-face manifold subsets, bounded segments/profile, overlap clamp, optional edge Float weights, generated-face material index, hardened smooth-face policy, deterministic transition/cap reconstruction, UV/material/attribute preservation, provenance, and live angle-selected Aetherfall runeblade | `bmo_bevel.cc`, `GEO_mesh_bevel.*`, `node_geo_mesh_bevel.cc`; Godot surface/material separation | Add custom profiles, offset modes, vertex-only bevel, miter policies, exact independent corner treatment for unequal incident weights, broader non-manifold policy, and interactive Studio preview/overlay |
| Named edge selection by adjacent-face angle | Implemented foundation | `select_edges_by_angle`; `rekall.modeling.selection.edge_angle`; reusable selection-set output consumed by bevel and modifier paths | Blender angle limit/field selection; generic Godot mesh-array boundary | Add grow/shrink, loop/ring, material, sharp, seam, island, arbitrary attribute predicate, viewport overlay, and selection history workflows |
...
|---|---|---|---|---|
| Displace/simple deform/lattice/curve deform/shrinkwrap | Partial | Wave 3: selection-aware taper and bend are available as semantic operations, graph nodes, and modifiers; Aetherfall's stored mantle graph is the first baked visible consumer | corresponding Blender simple-deform modifiers | Add twist, stretch, lattice, curve deform, shrinkwrap, Studio gizmos/preview, and broader animated consumers |
| Armature/skin/weights/constraints | Partial | Wave 3: point-domain joint indices/weights survive edit and Model Asset compilation; a versioned named-joint rig asset validates hierarchy/bind matrices, evaluates bind/pose/skin matrices, exposes revision-safe commands, and resolves `Rekall.RigPose` through the renderer. Generic animation clips now target stable native joint IDs with translation/rotation/scale channels, quaternion interpolation, and per-joint mixer blending; Aetherfall proves authored idle/walk clips change real rendered vertices | Blender armature modifier, pose channels, actions, and ordered constraints; Godot Mesh/Skin/Skeleton3D separation plus stable bone-name transform tracks in AnimationMixer | Add hierarchy/pose and curve/clip Studio editing, automatic/painted multi-joint normalize/prune tools, constraints/IK and foot planting, retargeting, root-motion extraction, debug visualization, and complete production-character acceptance |
