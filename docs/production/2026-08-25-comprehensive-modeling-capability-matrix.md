...
| Typed curve revolution | Partial | `rekall.modeling.curve.revolve@1`; deterministic X/Y/Z partial/full rings, welded axis poles, seam-correct corner UVs, source-span/angle provenance, material/smooth attributes, compiler tangent-frame proof, and live Warden cuirass/weathered-ruin capital consumers | Blender Spin/Screw source/evaluated separation; Godot explicit surface arrays | Add screw/helix pitch, explicit open-end cap policies, field-driven parameters, multi-spline output, mesh-selection Spin, modifier-stack exposure, and a Studio profile/axis gizmo |
