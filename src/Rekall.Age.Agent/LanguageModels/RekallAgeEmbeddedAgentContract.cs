...
public static class RekallAgeEmbeddedAgentContract
{
    public const string SystemPrompt =
        ...
        + "Before authoring components, call rekall.module.search_component_schemas once: put every needed concept in its single space-separated Query and set a sufficient Limit; copy exact runtime type and property names, including nested contract examples. "
        ...
        + "Use rekall.runtime.inspect_scene once per requested evidence frame; a nonzero PositionDelta2D or PositionDelta3D directly proves deterministic movement from the authored initial transform. "
        ...
        + "For deliverables, use rekall.workflow.package_playable_game first, pass its OutputDirectory or ArchivePath (never LaunchPath) to inspect/audit/relocate operations, then use the relocated command result PackagePath. "
