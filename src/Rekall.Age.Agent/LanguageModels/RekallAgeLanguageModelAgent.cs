        ...
        for (var turn = 1; turn <= maxTurns; turn++)
        {
            ...
            if (response.ToolCalls.Count == 0)
            {
                ...
                if (request.RequireCompletionAudit && !completionAuditPending)
                {
                    ...
                    transcript.Add(new RekallAgeLanguageModelMessage(
                        "user",
                        "Completion audit required. Treat your preceding response only as a proposal, not as accepted completion. Re-read the original task and direct tool evidence, and verify every explicit requirement. Zero counts, warnings or validation issues, missing components or artifacts, stale package proof after later mutations, and evidence that proves mere existence rather than the requested behavior are failures. Reuse direct passing evidence that remains current: do not repeat a passing operation merely to make it newer, do not recreate a passing package, and do not relocate an already proven relocated package again. Only call tools when a requirement is missing, contradicted, stale because of a later mutation, or not directly evidenced. If every requirement is directly proven, return the final evidence-backed response again. Do not rely on your prior narrative."));
                    continue;
                }
