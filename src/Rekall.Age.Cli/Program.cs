        ...
        CancellationToken cancellationToken)
    {
        ...
        var result = await agent.RunAsync(
            new RekallAgeLanguageModelAgentRequest(
                model,
                "You are the Rekall AGE embedded engine agent. Author arbitrary games through generic, inspectable engine tools and agent-owned C# modules. Start with engine status. Use rekall.tools.search to discover exact command names and full argument schemas, then call them through rekall.tools.execute. Before authoring components, call rekall.module.search_component_schemas once: put every needed concept in its single space-separated Query and set a sufficient Limit; copy exact runtime type and property names, including nested contract examples. Inspect results, execute diagnostic suggestedCommands exactly, repair failures, and prove the requested deliverables. Use rekall.runtime.inspect_scene for deterministic frame-based subsystem and entity-state inspection; it does not require a playable module. Use rekall.play.scene only when the task explicitly requires playable-module text frames or input playback. Honor explicitly named verification operations exactly; do not substitute interactive play, packaging, gauntlets, or unrelated workflows unless requested. For a new project, search for rekall.workflow.create_blueprint_project and submit all entities and all requested scenes through its Scenes array in one call; do not create a multi-scene project incrementally. For an existing scene, prefer rekall.scene.apply_blueprint in one complete declarative call instead of patching entities one at a time. For deliverables, use rekall.workflow.package_playable_game first, pass its OutputDirectory or ArchivePath (never LaunchPath) to inspect/audit/relocate operations, then use the relocated command result PackagePath. When package inspection, run, audit, and capture are all required, use rekall.workflow.audit_playable_package as the one consolidated inspect/run/nonblank-capture proof instead of spending calls on its separate sub-operations. Do not invent tool names or results and do not ask the engine to author game content for you.",
                task)
            {
                MaxTurns = maxTurns
            },
