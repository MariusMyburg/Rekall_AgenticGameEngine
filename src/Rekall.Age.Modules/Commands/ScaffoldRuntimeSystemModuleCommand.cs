    ...
    : IRekallAgeCommand<ScaffoldRuntimeSystemModuleRequest, ScaffoldRuntimeSystemModuleResult>
{
        ...
        string systemClass)
    {
        ...
        source.AppendLine("using System.Text.Json.Nodes;");
        source.AppendLine();
        ...
        source.AppendLine("        {");
        source.AppendLine("            var component = entity.Components.FirstOrDefault(item => item.Type == componentType);");
        source.AppendLine("            if (component is null || !ReadBoolean(component.Properties, \"enabled\", true))");
        source.AppendLine("            {");
        source.AppendLine("                return entity;");
        source.AppendLine("            }");
        source.AppendLine();
        source.AppendLine("            var valuePerSecond = ReadNumber(component.Properties, \"valuePerSecond\", 1);");
        source.AppendLine("            var transform = entity.Transform;");
        source.AppendLine("            return entity with");
        source.AppendLine("            {");
        source.AppendLine("                Transform = transform with");
        source.AppendLine("                {");
        source.AppendLine("                    Position3D = new RekallAgeRuntimeVector3(");
        ...
        source.AppendLine("    }");
        source.AppendLine();
        source.AppendLine("    private static bool ReadBoolean(JsonObject properties, string name, bool fallback)");
        source.AppendLine("    {");
        source.AppendLine("        return properties.TryGetPropertyValue(name, out var node)");
        source.AppendLine("            && node is JsonValue value");
        source.AppendLine("            && value.TryGetValue<bool>(out var boolean)");
        source.AppendLine("            ? boolean");
        source.AppendLine("            : fallback;");
        source.AppendLine("    }");
        source.AppendLine();
        source.AppendLine("    private static double ReadNumber(JsonObject properties, string name, double fallback)");
        source.AppendLine("    {");
        source.AppendLine("        return properties.TryGetPropertyValue(name, out var node)");
        source.AppendLine("            && node is JsonValue value");
        source.AppendLine("            && value.TryGetValue<double>(out var number)");
