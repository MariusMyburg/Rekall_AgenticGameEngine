...
public sealed class InspectSceneRuntimeCommand : IRekallAgeCommand<InspectSceneRuntimeRequest, InspectSceneRuntimeResult>
{
    ...
    public RekallAgeCommandSchema Schema => new(
        Name,
        "Inspects deterministic scene simulation after a requested frame count without requiring a compiled playable adapter; reports physics, animation, UI, audio, events, systems, and bounded entity states. For executable behavior proof, pass representative input frames plus 1-64 assertions. inputs[i] applies only to simulation frame i; omitted later frames receive neutral input, so repeat a held semantic sample for every frame it must remain down. Inject declared semantic actions with this exact frame shape: {\"semanticActions\":[{\"name\":\"move.horizontal\",\"value\":1,\"isDown\":true}]}; semantic samples override raw-device projection for the matching action declared by Rekall.InputActionMap. Assertion shape: {\"entityName\":\"Player\",\"subject\":\"component.property\",\"operator\":\"greater-than-or-equal\",\"componentType\":\"Game.Modules.Rules.PlayerState\",\"propertyName\":\"Score\",\"expected\":1}. Subjects: entity, visible, component, component.property, delta.component.property, changed.component.property, transform.position2d.x/y, transform.position3d.x/y/z, delta.position2d.x/y, delta.position3d.x/y/z. delta.component.property reports numeric final-minus-initial state and therefore requires that numeric property to be explicitly authored on the initial scene component; changed.component.property reports whether the bounded JSON value changed. Operators: exists, not-exists, equals, not-equals, contains, greater-than, greater-than-or-equal, less-than, less-than-or-equal. Any failed assertion fails the command with actual bounded values.",
