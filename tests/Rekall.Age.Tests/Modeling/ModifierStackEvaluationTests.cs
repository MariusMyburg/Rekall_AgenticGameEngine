...
public sealed class ModifierStackEvaluationTests
{
    ...
    public void CatalogDeclaresAttributePropagationAndLossPolicy()
    {
        ...
        Assert.Equal(16, catalog.Descriptors.Count);
        ...
        Assert.Contains(catalog.Descriptors, item => item.TypeId == "rekall.modifier.skin.linear_weights" && item.AttributePolicy.PreservesUnknownAttributes);
        Assert.Contains(catalog.Descriptors, item => item.TypeId == "rekall.modifier.skin.envelope_weights" && item.AttributePolicy.PreservesUnknownAttributes);
        Assert.Contains(catalog.Descriptors, item => item.TypeId == "rekall.modifier.deform.taper" && item.AttributePolicy.PreservesUnknownAttributes);
        Assert.Contains(catalog.Descriptors, item => item.TypeId == "rekall.modifier.deform.bend" && item.AttributePolicy.PreservesUnknownAttributes);
    }
